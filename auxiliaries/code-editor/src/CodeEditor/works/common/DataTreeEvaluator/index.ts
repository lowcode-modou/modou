...
export default class DataTreeEvaluator {
  ...
  setEvalTree(evalTree: DataTree) {
    console.log('setEvalTreesetEvalTree', evalTree)
  ...
  setupUpdateTree(unEvalTree: DataTree): {
    ...
    const { dependenciesOfRemovedPaths, extraPathsToLint, removedPaths } =
      updateDependencyMap({
  ...
  getDynamicValue(
    ...
    console.log(
      'getDynamicBindingsgetDynamicBindings',
      jsSnippets,
      stringSegments,
    )
...
// referencing DATA_BIND_REGEX fails for the value "{{Table1.tableData[Table1.selectedRowIndex]}}"
// if you run it multiple times and don't recreate
