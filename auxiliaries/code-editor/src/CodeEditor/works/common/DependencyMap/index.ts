...
export const updateDependencyMap = ({
  ...
  const dependenciesOfRemovedPaths: string[] = []
            ...
            if (Object.keys(entityDependencyMap).length) {
              ...
              Object.entries(entityDependencyMap).forEach(
                ([entityDependent, entityDependencies]) => {
                  ...
                  if (invalidReferences.length) {
                    dataTreeEvalRef.invalidReferencesMap[entityDependent] =
                      invalidReferences
                  } else {
            ...
            dataTreeEvalRef.invalidReferencesMap[path].forEach(
              (invalidReference) => {
                if (
                  ...
                  newlyValidReferencesMap[invalidReference] = mergeArrays(
                    newlyValidReferencesMap[invalidReference],
                    [path],
                  )
                ...
                if (validReferences.length) {
                  ...
                  if (dataTreeEvalRef.invalidReferencesMap[path]) {
                    const newInvalidReferences =
                      dataTreeEvalRef.invalidReferencesMap[path].filter(
                        (invalidReference) =>
                          // FIXME:(LiuLei)
                    ...
                    if (newInvalidReferences.length) {
                      dataTreeEvalRef.invalidReferencesMap[path] =
        ...
        case DataTreeDiffEvent.DELETE: {
          ...
          if (
            ...
            Object.keys(entityDependencies).forEach((widgetDep) => {
              didUpdateDependencyMap = true
              Reflect.deleteProperty(dataTreeEvalRef.dependencyMap, widgetDep)
              Reflect.deleteProperty(
                dataTreeEvalRef.invalidReferencesMap,
                widgetDep,
              )
          ...
          Object.keys(dataTreeEvalRef.dependencyMap).forEach(
            (dependencyPath) => {
              ...
              if (
                ...
                // Example scenario => For {{Api1.unknown}} in button.text,
                // if Api1 is deleted, we need to lint button.text
        ...
        case DataTreeDiffEvent.EDIT: {
          ...
          if (
            ...
            if (isADynamicBindingPath) {
              ...
              if (invalidReferences.length) {
                dataTreeEvalRef.invalidReferencesMap[fullPropertyPath] =
                  invalidReferences
              } else {
                Reflect.deleteProperty(
                  dataTreeEvalRef.invalidReferencesMap,
              ...
              if (correctSnippets.length) {
                dataTreeEvalRef.dependencyMap[fullPropertyPath] =
                  validReferences
              } else {
