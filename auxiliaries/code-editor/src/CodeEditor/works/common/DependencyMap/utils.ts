...
import { extractIdentifierInfoFromCode } from '@modou/ast'
// eslint-disable-next-line max-len
import { APPSMITH_GLOBAL_FUNCTIONS } from '@modou/code-editor/CodeEditor/components/editorComponents/ActionCreator/constants'
import {
...
export const extractInfoFromReferences = (
  ...
  references.forEach((reference: string) => {
    ...
    if (Reflect.has(allPaths, reference)) {
    ...
    while (subpaths.length > 1) {
      ...
      if (Reflect.has(allPaths, current)) {
 ...
 * NB: DATA_TREE_KEYWORDS in app/client/src/constants/WidgetValidation.ts isn't included,
 * although they are not valid entity names,
      ...
      ([path, entityDependencies]) => {
        ...
        if (Reflect.has(allPaths, mainPath)) {
          ...
          entityDependencies.forEach((dependentPath) => {
            ...
            if (Reflect.has(allPaths, completePath)) {
  ...
  if (isJSAction(entity)) {
    ...
    if (entity.reactivePaths) {
      Object.keys(entity.reactivePaths).forEach((propertyPath) => {
        const existingDeps = dependencies[`${entityName}.${propertyPath}`] || []
        const unevalPropValue = get(entity, propertyPath)
