...
import { isDynamicValue } from '@modou/code-editor/CodeEditor/utils/DynamicBindingUtils'
import { Types, getType } from '@modou/code-editor/CodeEditor/utils/TypeHelpers'

import { QUOTED_BINDING_REGEX } from '../../constants/bindings'
...
export const parameterSubstituteDynamicValues = (
  ...
  if (subSegments.length === 1 && subBindings.length === 1) {
    throw Error(
      'Dynamic bindings in prepared statements are only used to provide parameters inside SQL query.' +
        ' No SQL query found.',
    )
  }
