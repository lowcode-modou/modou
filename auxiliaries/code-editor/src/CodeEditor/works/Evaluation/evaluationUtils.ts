...
import { get, isFunction, isObject, isString, set, toPath } from 'lodash'
...
import { EvalProps } from '@modou/code-editor/CodeEditor/works/common/DataTreeEvaluator'
import { EvalMetaUpdates } from '@modou/code-editor/CodeEditor/works/common/DataTreeEvaluator/types'

import { DataTree } from '../../common/editor-config'
import { WidgetTypeConfigMap } from '../../utils/WidgetFactory'
import {
  DataTreeDiff,
  DataTreeDiffEvent,
  validateWidgetProperty,
} from '../common/DataTreeEvaluator/validationUtils'
...
export const resetValidationErrorsForEntityProperty = ({
  ...
  if (propertyPath) {
    ...
    const existingErrorsExceptValidation = (
      get(evalProps, errorPath, []) as EvaluationError[]
    ).filter(
      (error) => error.errorType !== PropertyEvaluationErrorType.VALIDATION,
    )
    set(evalProps, errorPath, existingErrorsExceptValidation)
  }
}
...
function translateDiffArrayIndexAccessors(
  propertyPath: string,
  lhs: any[],
  DELETE: any,
): any {
...
export function getSafeToRenderDataTree(
  ...
  return Object.keys(tree).reduce((tree, entityKey: string) => {
    ...
    Object.entries(entity.validationPaths).forEach(([property, validation]) => {
      ...
      set(safeToRenderEntity, property, parsed)
    })
    ...
    Object.keys(widgetTypeConfigMap[entity.type].derivedProperties).forEach(
      (property) => {
        set(safeToRenderEntity, property, undefined)
      },
    )
