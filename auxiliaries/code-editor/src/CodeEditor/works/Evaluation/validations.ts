...
import getIsSafeURL from '@modou/code-editor/CodeEditor/utils/validation/getIsSafeURL'
import {
  countOccurrences,
  findDuplicateIndex,
} from '@modou/code-editor/CodeEditor/works/Evaluation/helpers'
...
const LINE_BREAKS_ERROR_MESSAGE =
  `Warning: New lines in the text exceed ${MAX_ALLOWED_LINE_BREAKS}.` +
  `The text displayed will not contain any new lines.`

const flat = (array: Array<Record<string, any>>, uniqueParam: string) => {
...
function validatePlainObject(
  ...
  if (config.params?.allowedKeys) {
    ...
    config.params.allowedKeys.forEach((entry) => {
      ...
      if (Reflect.has(value, entryName)) {
...
function validateArray(
  ...
  uniqueKeys = Array.isArray(config.params?.unique)
    ? uniqueKeys.concat(config.params?.unique as string[])
    : uniqueKeys
      ...
      return {
        ...
        messages: [
          `Duplicate values found for the following properties,` +
            ` in the array entries, that must be unique -- ${uniqueKeys.join(
              ',',
            )}.`,
        ],
      }
...
export const VALIDATORS: Record<ValidationTypes, Validator> = {
  [ValidationTypes.TEXT]: (
    ...
    if (isObject(value)) {
      if (config.params?.limitLineBreaks && validateExcessLineBreaks(value)) {
        return {
          isValid: false,
  ...
  [ValidationTypes.ARRAY]: (
    ...
    if (value === undefined || value === null || value === '') {
      if (config.params?.required && !isArray(config.params.default)) {
