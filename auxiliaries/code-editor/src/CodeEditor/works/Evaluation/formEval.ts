...
import {
  EvaluatedFormConfig,
  FormEvalOutput,
} from '@modou/code-editor/CodeEditor/reducers/evaluationReducers/formEvaluationReducer'
...
const generateInitialEvalState = (formConfig: FormConfigType) => {
  ...
  // Any element is only added to the eval state if they have a conditional statement present,
  // if not they are allowed to be rendered
  if ('conditionals' in formConfig && !!formConfig.conditionals) {
...
function generateEvalFormConfigPaths(
  ...
  // we also never check children and schema cause the recursive function that this function
  // is called in already checks the children and schemas (to prevent double recursive checks).
  // the second placeHolderText is due to a rogue value in the formConfig of one of
  // S3 datasource form config.
        ...
        if (!!conditionBlock) {
          Object.keys(conditionBlock).forEach((conditionType: string) => {
            ...
            if (conditionType === ConditionType.HIDE) {
              ...
              // If the key in the currentEval state has dynamicDependencyPathList,
              // we check to see if the path of the changed value
              // exists in the path list, if it does, we evaluate
              // the dynamicValues and fetch the data via API call,
              // but if the value does not exist in the path list,
              // we prevent the dynamic value from being refetched via API call.
              // in other words, if the current actionDiffPath is a dependency,
              // then isActionDiffADependency becomes true.
              if (
              ...
              // if the actionDiffPath is a dependency or if the route has changed
              // (navigated to another action/page) of if there's no actionDiffPath at all (when the page is refreshed)
...
function getFormEvaluation(
  ...
  if (!!currentEvalState && currentEvalState.hasOwnProperty(formId)) {
    ...
    for (const [key, value] of Object.entries(currentFormIdEvalState)) {
      ...
      // static dependency pathlist should be a key of identifiers that point to
      // formControls that are dependent on the result of the current form config value.
      // it is important to note the difference between staticDependencyPathList
      // and dynamicDependencyPathList is that the former is for formConfigs that don't
      // require API calls.
