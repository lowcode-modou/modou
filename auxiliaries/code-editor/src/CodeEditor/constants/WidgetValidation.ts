 ...
 * These identifiers are already present in the worker context and shouldn't
 * represent any valid identifier within Appsmith, as no entity should have
 * same name as them to prevent unexpected behaviour during evaluation(which happens on
 * the worker thread) in the worker.
 * Check if an identifier (or window object/property) is available in the worker context
 * here => https://worker-playground.glitch.me/
 */
